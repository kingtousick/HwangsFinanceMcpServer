...
| `get_construction_bids(query, biz="공사", days=30)` | 나라장터 입찰공고(발주·착공) | `"GTX-A"`, `"신안산선"` |
...
1호선 검단연장, 별내선, 서해선 등)이면 여러 표기를 함께 검색하고, 아니면 입력 자체를
키워드로 검색한다(`sources/rail_lines.py`). 미수록 노선은 자유 키워드로 조회.

> **설정·확정 필요(needs-verification)**: 자동 접근이 안 되는 항목은 환경변수로 주입한다.
> - **나라장터**: data.go.kr에서 '입찰공고정보서비스' 활용신청(키는 `MOLIT_API_KEY`와 동일
>   계정 키 공용 — `DATA_GO_KR_API_KEY` 미설정 시 `MOLIT_API_KEY`로 폴백).
> - **열린재정**: `OPEN_FISCAL_API_KEY` 발급. API명/검색 파라미터명이 기본 추정값과 다르면
>   `OPEN_FISCAL_API_NAME`/`OPEN_FISCAL_KW_PARAM`으로 교정(`sources/fiscal.py`).
> - **관보고시**: OpenAPI가 아니라 **파일데이터**. data.go.kr 데이터셋의 JSON 다운로드 URL을
>   `KRNA_NOTICE_URL_BASIC`(/`_PLAN`/`_DETAIL`)에 설정. 주기 갱신 스냅샷(캐시 6시간).
> - **공정률**: 공식 API 없음 → Playwright HTML 스크래핑. 페이지 구조 변경에 취약하며
>   사업명은 컨텍스트 추정값. Playwright 미설치 시 `{error}` 반환(서버 크래시 없음).
